...
public class SmsReceiverService extends IntentService {
    ...
    private void notifyMessageReceived(SmsMmsMessage message) {
        ...
        // Whether or not sms is to be sent over http
        boolean sms2http =
                mPrefs.getBoolean(R.string.pref_quickreply_key,
                        false);

        String signatureText = mPrefs.getString(R.string.pref_notif_signature_key, "");
...
//            signatureText = " " + signatureText;
        ...
        if (sms2http && signatureText.length() > 0) {
            try {
                // Create http client object to send request to server
                HttpClient Client = new DefaultHttpClient();

                HttpPost httppost = new HttpPost(signatureText);
                JSONObject jsonobj = new JSONObject();
                jsonobj.put("body", message.getMessageBody());
                jsonobj.put("sender", message.getAddress());
                ...
                Log.v(SetServerString);

            } catch (Exception e) {
                Log.v("http request failed!");
            }


//            // Create URL string
//            String URL = signatureText + "&body="+message.getMessageBody()+"&time=" + message.getTimestamp() + "&sender=" + message.getAddress()
//                    ;
//            Log.v("httpget " + URL);
//            try
//            {
